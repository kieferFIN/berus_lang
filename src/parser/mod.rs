...
fn parse_function_def(input: &str)-> IResult<&str, (String,FunctionDef)>{
    let(input,_) = tuple((multispace0,tag("fn"),multispace1))(input)?;
    let (input,name) = parse_name(input)?;
    let(input,_) = tuple((multispace0, tag("(")))(input)?;
    let(input,parameters) = separated_list0(tag(","),pair(parse_name,parse_type_info))(input)?;
    let(input,_) = tuple((multispace0, tag(")"),multispace0,tag(";")))(input)?;
    Ok((input,(name, FunctionDef{parameters, return_type:None})))
...
fn parse_type_info(input: &str) -> IResult<&str,TypeInfo>{
    let(input, type_name) = preceded(tag(":"), parse_name)(input)?;
...
fn parse_constant_value(input: &str) -> IResult<&str, ConstantValue>{
    ...
    let integer_value = if neg.is_some(){
        integer_value * -1
    }else {
        integer_value
    };
    ...
    let value = match end {
        Some(end) => ConstantValue::Float(end+integer_value as f32),
        None => ConstantValue::Integer(integer_value)
    };
    Ok((input, value))
}

fn parse_name(input: &str) ->IResult<&str,String>{
    //TODO: check that first char is alpha !!
